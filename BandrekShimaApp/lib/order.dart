...
class Order extends StatefulWidget {
  final token;
  const Order({@required this.token,Key? key}) : super(key: key);
...
class _OrderState extends State<Order> {
  ...
  void getCartList(userId) async {
    try {
      var url = Uri.parse('$getCheckoutUserCart?userId=$userId');
  ...
  void _calculateTotalPrice() {
    if (items != null) {
      ...
      for (int i = 0; i < items!.length; i++) {
        total += int.parse(items![i]['price'].toString());
      }
      setState(() {
        _totalPrice = total;
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 8.0),
                Text('Apa Kabar? Ayo Lihat Pesanan Kamu',style: TextStyle(fontSize: 20),),
              ...
              child: Padding(
                ...
                child: items == null || items!.isEmpty
                  ...
                  child: Column(
                    ...
                    children: [
                      Image.asset(
                        'img/order.png', // Replace with your image asset path
                        width: 150,
                        height: 150,
                        fit: BoxFit.contain,
                      ),
                      SizedBox(height: 20),
                      Text('Pesanan Kamu Kosong', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                    ],
                  ),
                )
                    : ListView.builder(
                    ...
                    itemBuilder: (context,int index){
                      return Slidable(
                        ...
                        child: Card(
                          ...
                          child: ListTile(
                            contentPadding: EdgeInsets.symmetric(vertical: 8.0), // Sesuaikan sesuai kebutuhan
                            leading: _getProductIcon(items![index]['name']),
                            title: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('${items![index]['status']}', style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      floatingActionButton: items == null || items!.isEmpty
        ...
        onPressed: () {
          // Implementasi dialog dan penambahan item
          // ...

          // Setelah menambahkan item, perbarui total harga
          _calculateTotalPrice();
        },
        label: Text('Total Harga: Rp $_totalPrice'),
        tooltip: 'Add-ToDo',
      ),
        ...
        child: Row(
          ...
          children: [
            ...
            IconButton(
              onPressed: () {
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => Order(token: widget.token)));                // Add your action here
              },
